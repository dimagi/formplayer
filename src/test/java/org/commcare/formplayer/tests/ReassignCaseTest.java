...
public class ReassignCaseTest extends BaseTestClass {
    ...
    public void testCaseReassign() throws Exception {
        ...
        UserSqlSandbox sandbox = new UserSqlSandbox(
                getUserDbConnector("reassigndomain", "reassignusername", null));
        SqlStorage<Case> caseStorage = sandbox.getCaseStorage();

        assert caseStorage.getNumRecords() == 0;

        NewFormResponse formResponse = sessionNavigate(new String[]{"0"}, "reassign",
        ...
        EntityListResponse entityListResponse = sessionNavigate(new String[]{"1", "2"}, "reassign",
                EntityListResponse.class);
        assert entityListResponse.getEntities().length == 1;
        String caseId = entityListResponse.getEntities()[0].getId();

        NewFormResponse formResponse2 = sessionNavigate(new String[]{"1", "2", caseId}, "reassign",
                NewFormResponse.class);
        ...
        EntityListResponse entityListResponse2 = sessionNavigate(new String[]{"1", "2"}, "reassign",
                EntityListResponse.class);
