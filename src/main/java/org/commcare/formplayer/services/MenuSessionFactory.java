...
public class MenuSessionFactory {
    ...
    public void rebuildSessionFromFrame(MenuSession menuSession, CaseSearchHelper caseSearchHelper) throws CommCareSessionException {
        OrderedHashtable oldData = (OrderedHashtable) menuSession.getSessionWrapper().getData().clone();
        System.out.println("Rebuilding session, which has keys " + menuSession.getSessionWrapper().getData().keySet());
                ...
                for (StackFrameStep step : steps) {
                    if (step.getId().equals(neededDatum.getDataId())) {
                        ...
                        try {
                            ...
                            if (neededDatum.getDataId().equals("results")) {
